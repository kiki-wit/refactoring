...
>설명이 길고 복잡했지만, 간단히 말해서
git으로 소스를 관리하면서 계속 통합해주고
그와중에 자가 테스트 코드를 마련하고 
리팩터링을 끊임없이 해주어야 한다는 말이다.
<br/>
마지막 문단의 말이 핵심인 것 같다. 결국 <br/>
<b>어떠한 접근법이든지 충분한 연습과 실력이 뒷받침돼야 한다.</b>
...
>챕터2에서는 전반적으로 리팩터링의 개념과 목적, 리팩터링하는 방법에 대해 설명하고 있다. 
>중복 코드처럼 몇 장에 걸쳐 반복적으로 설명하고 있는 내용도 있는데, 리팩터링의 실제 목적이다.
><b>리팩터링의 궁극적인 목적은 경제 가치를 창출하는 것이다.</b>
>리팩터링의 목적이 단순히 "보기좋은 코드"는 아니라는 것이다.
>나 또한 여태까지 리팩터링을 단순히 "보기 좋은", "유지 보수를 잘하기 위한" 도구로 생각하고
>있었다. 1판이 나온지도 10년이 됐는데 아직도 잘못 알고 있는 사람이 있으니 중요하게 생각하고 
>반복해서 설명할만 하다는 생각이 들었다.
> 챕터1에서는 리팩터링을 하면서 성능은 신경쓰지 말라고 했다.
>챕터2에서는 그 이유를 설명한다. 사실 개발하면서 좋은 코드도 중요하지만 성능 또한 중요한 부분 중 하나로
>체감을 많이 느끼는 부분이기 때문에 성능을 고려하지 말라는 말은 맞지 않다는 생각이 들었었다. 
>챕터2에서는 리팩터링을 통해 성능을 개선할 수 있는 방법을 알려준다.
>성능은 생략한 코드를 짜고 컴파일과 테스트, 프로파일러 실행 과정을 거쳐서 
>문제 지점을 찾아내고 최적화 될 때까지 반복하는 것이다.
>이렇게 하면 오히려 나중에 문제가 생길 부분이 미리 파악할 수 있고 리팩터링으로 
>좋은 코드를 만들 수도 있다.
>
>챕터2를 읽고 두루뭉술하게 느끼고 있었던 리팩터링이란 무엇인가? 에 대한 답이 분명해지고 
>잘못 알고 있었던 개념들도 바로잡을 수 있었다.
