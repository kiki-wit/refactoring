...
><br/>리팩터링은 성능 최적화와 비슷하다. 둘 다 코드를 변경하지만 프로그램의 전반적인 기능은 그대로 유지한다. 단지 목적이 다를 뿐이다. 리팩터링의 목적은 코드를 이해하고 수정하기 쉽게 만드는 것이다.
반면 성능 최적화는 오로지 속도 개선에만 신경 쓴다.
<br/>

#### 2.2 두 개의 모자

><br/>켄트 백은 소프트웨어를 개발 할 때 두 개의 모자에 비유했다.
...
#### 2.4 언제 리팩터링해야 할까?
##### 3의 법칙
><br>돈 로버츠(Don Roberts)가 제시한 가이드 
>1. 처음에는 그냥 한다.
...
<aside style="background:#dede;padding:10px">
최근 팀 내부에서 팀장님과 계층형 게시판을 짜면서 나란히 앉아서 리팩토링을 하고 있는데 코드가 점점 개선되고 깔끔해지는 것을 경험했습니다.
<br/>
혼자서 리팩토링을 하게 되면 어떤 점이 문제인지는 알아도 어떻게 고쳐나가야 할지 헷갈릴 때가 많은데 리뷰어의 말에 따라 개선해가면서 그 부분이 명확해지는 느낌이 들었습니다.
</aside>
...
<aside style="background:#dede;padding:10px">
저자가 몇 장에 걸쳐서 리팩터링의 목적을 설명하는 것을 보고 많이 답답했나? 하는 생각이 들었습니다.<br/>
저도 리팩토링을 하는 목적이 단순히 개발자가 보기 좋고 유지보수 하기 좋은 질 좋은 코드로 개선하는 것이다라고 착각하고 있었는데 궁극적으로 경제적인 효과를 바라보고 있다는 것을 새롭게 배우게 되었습니다.
</aside>
